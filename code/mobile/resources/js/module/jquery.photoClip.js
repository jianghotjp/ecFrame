 ...
 * @option_param {number} rate 不截取时绘制质量,默认0.7
 * @option_param {number} clipRate 截取时绘制质量,默认0.7
 * @option_param {boolean} nocilp 是否截取，默认false，截取
...
(function(root, factory) {
...
}(this, function($j, IScroll, Hammer, lrz) {
	"use strict";

	$j.fn.photoClip = function(option) {
	...
	function photoClip(container, option) {
		...
		$file.change(function() {
			...
			//if (!/image\/\w+/.test(files.type)) {  //某些机型有问题，前面做了限制，去掉匹配type
			//if(false){
		...
		if ($ok.length) {
			$ok.on('tap',function(){
				var class_name = $ok[0].className;
				if(class_name.search('readDone') != -1){
                    clipImg();
                }
			})
		}
		...
		function clipImg() {
			...
			var dataURL = canvas.toDataURL(outputType, clipRate);
		...
		function zipImg(){
            ...
            if(imgWidth<750){
            	...
            	// 压缩宽度可提出
                canvas.width = 750;
                canvas.height = Math.floor(750*imgHeight/imgWidth);
			}
            ctx.clearRect(0, 0, canvas.width, canvas.height);
            ctx.save();
            ...
            var dataURL = canvas.toDataURL(outputType,rate);
            clipFinish.call($img[0], dataURL);
		}
		...
		function init() {
			...
			// 创建裁剪视图层
			$clipView = $j("<div class='photo-clip-view'>").css({
				"position": "absolute",
				"left": "50%",
				"top": "50%",
				"width": clipWidth,
			...
			$moveLayer = $j("<div class='photo-clip-moveLayer'>").appendTo($clipView);

			$rotateLayer = $j("<div class='photo-clip-rotateLayer'>").appendTo($moveLayer);

			// 创建遮罩
			var $mask = $j("<div class='photo-clip-mask'>").css({
				"position": "absolute",
				"left": 0,
				"top": 0,
				"width": "100%",
			...
			var $mask_left = $j("<div class='photo-clip-mask-left'>").css({
				"position": "absolute",
				"left": 0,
				"right": "50%",
				"top": "50%",
			...
			var $mask_right = $j("<div class='photo-clip-mask-right'>").css({
				"position": "absolute",
				"left": "50%",
				"right": 0,
				"top": "50%",
			...
			var $mask_top = $j("<div class='photo-clip-mask-top'>").css({
				"position": "absolute",
				"left": 0,
				"right": 0,
				"top": 0,
			...
			var $mask_bottom = $j("<div class='photo-clip-mask-bottom'>").css({
				"position": "absolute",
				"left": 0,
				"right": 0,
				"top": "50%",
			...
			// 创建截取区域
			var $clip_area = $j("<div class='photo-clip-area'>").css({
